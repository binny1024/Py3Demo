...
def crawler():
    try:
        ...
        for index in range(0, tx_lists_size):
            tx_hash = selenium_util.find_child_element_by_class_name(tx_lists[index], 'address').text
            print('hash : ' + tx_hash)

            tx_time = selenium_util.find_child_element_by_class_name(tx_lists[index], 'time').text
            print("date : " + tx_time)

            tx_from_element = selenium_util.find_child_element_by_class_name(tx_lists[index], 'from')
            tx_from_text = selenium_util.find_child_element_by_class_name(tx_from_element, 'address').text
            print('from : ' + tx_from_text)

            tx_to_element = selenium_util.find_child_element_by_class_name(tx_lists[index], 'to')
